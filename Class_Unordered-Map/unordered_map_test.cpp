...
void TestIterators() {
    ...
    for (double d = 100.0; d < 10100.0; d += 0.1) {  // NOLINT
        m.emplace(d, "a");
    }
    ...
    // Must be also fast
    for (double d = 200.0; d < 10200.0; d += 0.35) {  // NOLINT
        auto it = m.find(d);
        if (it != m.end()) {
            m.erase(it);
        }
...
TestConstIteratorDoesntAllowModification(T /*unused*/) {
    assert(false);
}
template <typename... FakeArgs>
void TestConstIteratorDoesntAllowModification(FakeArgs... /*unused*/) {}
...
struct TheChosenOne : public std::allocator<T> {
    ...
    template <typename U>
    TheChosenOne(const TheChosenOne<U>& /*unused*/) {}
